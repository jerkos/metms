...
class MSBaseController(object, WithPointerToQApp): 
    ...
    def __init__(self, model, view):
        ...
        """
        #QObject.__init__(self, parent=self.qApp)
        self.view = view
        self.model = model
    ...
    def filesFromView(self, *args):
        ...
        for index in self.acTree.selectedIndexes():
            if index.isValid():
                xmlfile = index.data().toString()#self.acModel.data(index, Qt.DisplayRole).toString()
...
class MSPaintingCapaciter(MSBaseController):
    """
    base imlementing plotting capabilities
    """    
    pass
...
#                                                        #<p>Select a tab corresponding to the action you want""")
    ...
    def _buildSampleList(self, creation=True):#, **kw):
        """
        to describe
            ...
            if self.selectedFiles:
                for xml in self.selectedFiles:
                    if str(xml) not in self.model.getFiles():
                        print xml
                        sample = obj.MSSample(xmlfile=self.goodName(str(xml))) #with of not with xmlfile keyword does not change anything
