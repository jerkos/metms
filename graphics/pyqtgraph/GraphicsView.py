...
class GraphicsView(QtGui.QGraphicsView):
    ...
    sigRangeChanged = QtCore.pyqtSignal(object, object)
    sigMouseReleased = QtCore.pyqtSignal(object)
    sigSceneMouseMoved = QtCore.pyqtSignal(object)
    ...
    def __init__(self, parent=None, useOpenGL=True):
        ...
        if useOpenGL:
            v = QtOpenGL.QGLWidget(QtOpenGL.QGLFormat(QtOpenGL.QGL.SampleBuffers))
        else:
            v = QtGui.QWidget()
        self.setViewport(v)
...
#        self.setPalette(palette)
    ...
    def addText(self, *args, **kw):
        item = self.scene().addText(*args, **kw)
        item.setPos(QtCore.QPointF(0.,100.))
        return item
    ...
    def wheelEvent(self, ev):
        ...
        #self.scale *= sc
        #self.updateMatrix()
        self.scale(1., sc, center=self.mapToScene(QCursor.pos()))
        self.sigRangeChanged.emit(self, self.range)
    ...
    def mouseMoveEvent(self, ev):
        ...
        #QtGui.QToolTip.showText(QtCore.QPoint(int(ev.pos().x()),int(ev.pos().y()) ), 
        #                        ",".join([str(ev.pos().x()), str(ev.pos().y())]))
